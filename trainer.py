"""
trainer.py  —  Orchestrator for Sig-Bridge Engine
==================================================

Loads data → computes bridges → builds JSON → uploads.
...
from sig_bridge import compute_universe_sig_bridge
...
def process_window(args: Tuple) -> Dict:
    ...
    try:
        ...
        result = compute_universe_sig_bridge(universe_prices, macro_df, config_dict, window)
...
def run_trainer(hf_token: Optional[str] = None) -> Dict:
    """Run the full Sig-Bridge pipeline."""
    ...
    # ── Save JSON ─────────────────────────────────────────────────────────────
    logger.info("\n💾 Saving JSON results...")
    tab1_path = f"sig_bridge_{run_date}.json"
    tab2_path = f"sig_bridge_breakdown_{run_date}.json"
